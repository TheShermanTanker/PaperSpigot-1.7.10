...
/*  226 */   private int stackSize = 0; private JsonToken token; private String name; private String value; private int valuePos; private int valueLength; private boolean skipping;
...
/*      */     
/*      */     try {
/*  559 */       l = Long.parseLong(this.value);
...
/*      */ 
/*      */     
/*  569 */     if (l >= 1L && this.value.startsWith("0")) {
...
/*      */     
/*      */     try {
/*  597 */       i = Integer.parseInt(this.value);
...
/*      */ 
/*      */     
/*  607 */     if (i >= 1L && this.value.startsWith("0")) {
...
/*  744 */         return this.token = JsonToken.NAME; }  checkLenient(); this.pos--; this.name = nextLiteral(false); if (this.name.length() == 0) throw syntaxError("Expected name");  this.stack[this.stackSize - 1] = JsonScope.DANGLING_NAME; return this.token = JsonToken.NAME;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*      */       case 61:
/*      */         checkLenient(); if ((this.pos < this.limit || fillBuffer(1)) && this.buffer[this.pos] == '>')
/*      */           this.pos++; 
/*      */     } 
/*  771 */     throw syntaxError("Expected ':'"); } private JsonToken nextValue() throws IOException { int c = nextNonWhitespace(true);
/*  772 */     switch (c) {
/*      */       case 123:
...
/*  805 */     for (int i = 0, p = this.pos; i < p; i++) {
/*  806 */       if (buffer[i] == '\n') {
/*  807 */         line++;
/*  808 */         column = 1;
/*      */       } else {
...
/*      */ 
/*      */       
/*  829 */       if (this.bufferStartLine == 1 && this.bufferStartColumn == 1 && this.limit > 0 && buffer[0] == '﻿') {
/*  830 */         this.pos++;
/*  831 */         this.bufferStartColumn--;
/*      */       } 
/*      */       
...
/*  873 */     int p = this.pos;
/*  874 */     int l = this.limit; while (true) {
/*      */       char peek;
/*  876 */       if (p == l) {
/*  877 */         this.pos = p;
/*  878 */         if (!fillBuffer(1)) {
...
/*  972 */     for (; this.pos + toFind.length() <= this.limit || fillBuffer(toFind.length()); this.pos++) {
/*  973 */       int c = 0; while (true) { if (c < toFind.length()) {
...
/*      */           case '=':
/*      */           case '\\':
/* 1064 */             checkLenient(); break label43;
/*      */           case '\t': break label43;
/*      */           case '\n':
/*      */             break label43;
/*      */           case '\f':
...
/*      */             break label43;
/*      */           case '}':
/* 1085 */             break label43; }  }  if (i < this.buffer.length) {
/* 1086 */         if (fillBuffer(i + 1)) {
/*      */           continue;
/*      */         }
/* 1089 */         this.buffer[this.limit] = Character.MIN_VALUE;
/*      */ 
/*      */         
/*      */         break;
/*      */       } 
...
/* 1110 */       result = null;
/* 1111 */     } else if (this.skipping) {
/* 1112 */       result = "skipped!";
/* 1113 */     } else if (builder == null) {
...
/*      */         }
/*      */         
/* 1149 */         result = Character.MIN_VALUE;
/* 1150 */         for (i = this.pos, end = i + 4; i < end; i++) {
/* 1151 */           char c = this.buffer[i];
/* 1152 */           result = (char)(result << 4);
/* 1153 */           if (c >= '0' && c <= '9') {
/* 1154 */             result = (char)(result + c - 48);
/* 1155 */           } else if (c >= 'a' && c <= 'f') {
/* 1156 */             result = (char)(result + c - 97 + 10);
/* 1157 */           } else if (c >= 'A' && c <= 'F') {
/* 1158 */             result = (char)(result + c - 65 + 10);
/*      */           } else {
...
/* Location:              D:\Paper-1.7.10\PaperSpigot-1.7.10-R0.1-SNAPSHOT-latest.jar!\org\bukkit\craftbukkit\libs\com\google\gson\stream\JsonReader.class
