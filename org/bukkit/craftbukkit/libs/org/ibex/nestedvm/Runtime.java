...
/*   44 */     return 0; } protected int userInfoSize() {
/*   45 */     return 0;
/*      */   }
/*      */ 
/*      */ 
...
/*      */ 
/*      */   
/*   67 */   protected int state = 1; private CallJavaCB callJavaCB; private byte[] _byteBuf; static final int MAX_CHUNK = 16776192; static final boolean win32Hacks;
...
/*   82 */     this.sm = paramSecurityManager;
/*      */   }
/*      */   
/*      */   public void setCallJavaCB(CallJavaCB paramCallJavaCB) {
/*   86 */     this.callJavaCB = paramCallJavaCB;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  136 */     this(paramInt1, paramInt2, false);
...
/*      */     
/*  142 */     byte b = 0;
/*  143 */     for (; paramInt1 >>> b != 1; b++);
/*  144 */     this.pageShift = b;
/*      */     
/*  146 */     int i = heapStart();
/*  147 */     int j = paramInt2 * paramInt1;
/*  148 */     int k = max(j / 512, 131072);
/*  149 */     int m = 0;
/*  150 */     if (paramInt2 > 1) {
/*  151 */       k = max(k, paramInt1);
/*  152 */       k = k + paramInt1 - 1 & (paramInt1 - 1 ^ 0xFFFFFFFF);
/*  153 */       m = k >>> this.pageShift;
/*  154 */       i = i + paramInt1 - 1 & (paramInt1 - 1 ^ 0xFFFFFFFF);
/*  155 */       if (m + 4 + (i >>> this.pageShift) >= paramInt2)
/*  156 */         throw new IllegalArgumentException("total pages too small"); 
/*      */     } else {
/*  158 */       if (paramInt1 < i + k) throw new IllegalArgumentException("total memory too small"); 
/*  159 */       i = i + 4095 & 0xFFFFEFFF;
/*      */     } 
/*      */     
/*  162 */     this.stackBottom = j - k;
/*  163 */     this.heapEnd = i;
/*      */     
...
/*      */     } else {
/*  171 */       for (int n = this.stackBottom >>> this.pageShift; n < this.writePages.length; n++) {
/*  172 */         this.writePages[n] = new int[paramInt1 >> 2]; this.readPages[n] = new int[paramInt1 >> 2];
/*      */       } 
/*      */     } 
/*      */     
/*  176 */     if (!paramBoolean) {
...
/*      */       
/*  180 */       InputStream inputStream = win32Hacks ? new Win32ConsoleIS(System.in) : System.in;
/*  181 */       addFD(new TerminalFD(inputStream));
...
/*  253 */         for (byte b1 = 0; b1 < i1; b1++, b += 4) {
/*  254 */           int i2 = arrayOfInt[n + b1];
/*  255 */           paramArrayOfbyte[b + 0] = (byte)(i2 >>> 24 & 0xFF); paramArrayOfbyte[b + 1] = (byte)(i2 >>> 16 & 0xFF);
/*  256 */           paramArrayOfbyte[b + 2] = (byte)(i2 >>> 8 & 0xFF); paramArrayOfbyte[b + 3] = (byte)(i2 >>> 0 & 0xFF);
/*      */         } 
/*  258 */         m += i1; k -= i1;
/*      */       } 
...
/*  281 */       int k = memRead(paramInt1 & 0xFFFFFFFC);
/*  282 */       switch (paramInt1 & 0x3) { case 1:
/*  283 */           k = k & 0xFF00FFFF | (paramArrayOfbyte[b++] & 0xFF) << 16; if (--paramInt2 == 0)
/*  284 */             break; case 2: k = k & 0xFFFF00FF | (paramArrayOfbyte[b++] & 0xFF) << 8; if (--paramInt2 == 0)
/*  285 */             break; case 3: k = k & 0xFFFFFF00 | (paramArrayOfbyte[b++] & 0xFF) << 0; if (--paramInt2 == 0); break; }
/*      */       
/*  287 */       memWrite(paramInt1 & 0xFFFFFFFC, k);
/*  288 */       paramInt1 += b;
/*      */     } 
/*      */     
/*  291 */     if ((paramInt2 & 0xFFFFFFFC) != 0) {
/*  292 */       int k = paramInt2 >>> 2;
/*  293 */       int m = paramInt1 >>> 2;
/*  294 */       while (k != 0) {
/*  295 */         int[] arrayOfInt = this.writePages[m >>> this.pageShift - 2];
/*  296 */         if (arrayOfInt == null) throw new WriteFaultException(m << 2); 
/*  297 */         int n = m & j;
/*  298 */         int i1 = min(k, i - n);
/*  299 */         for (byte b1 = 0; b1 < i1; b1++, b += 4)
/*  300 */           arrayOfInt[n + b1] = (paramArrayOfbyte[b + 0] & 0xFF) << 24 | (paramArrayOfbyte[b + 1] & 0xFF) << 16 | (paramArrayOfbyte[b + 2] & 0xFF) << 8 | (paramArrayOfbyte[b + 3] & 0xFF) << 0; 
/*  301 */         m += i1; k -= i1;
/*      */       } 
/*  303 */       paramInt1 = m << 2; paramInt2 &= 0x3;
/*      */     } 
/*      */     
/*  306 */     if (paramInt2 != 0) {
/*  307 */       int k = memRead(paramInt1);
/*  308 */       switch (paramInt2) { case 1:
/*  309 */           k = k & 0xFFFFFF | (paramArrayOfbyte[b + 0] & 0xFF) << 24; break;
/*  310 */         case 2: k = k & 0xFFFF | (paramArrayOfbyte[b + 0] & 0xFF) << 24 | (paramArrayOfbyte[b + 1] & 0xFF) << 16; break;
/*  311 */         case 3: k = k & 0xFF | (paramArrayOfbyte[b + 0] & 0xFF) << 24 | (paramArrayOfbyte[b + 1] & 0xFF) << 16 | (paramArrayOfbyte[b + 2] & 0xFF) << 8; break; }
/*      */       
/*  313 */       memWrite(paramInt1, k);
/*      */     } 
/*      */   }
/*      */   
...
/*      */ 
/*      */   
/*      */   public final void memWrite(int paramInt1, int paramInt2) throws WriteFaultException {
/*  419 */     if ((paramInt1 & 0x3) != 0) throw new WriteFaultException(paramInt1); 
/*  420 */     unsafeMemWrite(paramInt1, paramInt2);
/*      */   }
/*      */   
/*      */   protected final void unsafeMemWrite(int paramInt1, int paramInt2) throws WriteFaultException {
...
/*  437 */     return initPage(paramInt, false);
/*      */   }
...
/*  480 */     if (paramArrayOfString == null) paramArrayOfString = new String[0];  return paramArrayOfString;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  537 */   public final int run(String paramString, String[] paramArrayOfString) { return run(concatArgv(paramString, paramArrayOfString)); } public final int run(String[] paramArrayOfString) {
/*  538 */     return run(paramArrayOfString, (String[])null);
/*      */   }
/*      */ 
/*      */   
...
/*      */   }
/*      */   
/*  552 */   public final void start() { start(null); } public final void start(String[] paramArrayOfString) {
/*  553 */     start(paramArrayOfString, null);
/*      */   }
/*      */   
...
/*  576 */       byte b = (this.writePages.length == 1) ? 4096 : (1 << this.pageShift);
/*  577 */       this.heapEnd = this.heapEnd + b - 1 & (b - 1 ^ 0xFFFFFFFF);
/*      */     } 
/*      */     
...
/*      */   }
/*      */   
/*  637 */   public final int call(String paramString) throws CallException { return call(paramString, new int[0]); }
/*  638 */   public final int call(String paramString, int paramInt) throws CallException { return call(paramString, new int[] { paramInt }); } public final int call(String paramString, int paramInt1, int paramInt2) throws CallException {
/*  639 */     return call(paramString, new int[] { paramInt1, paramInt2 });
/*      */   }
/*      */   
...
/*  744 */       throw new ErrnoException(134);
/*      */     } 
/*  746 */     boolean bool = ((paramInt1 & 0x3) != 0) ? true : false;
/*      */     
...
/*      */ 
/*      */       
/*  764 */       try { file = new Seekable.File(paramFile, bool, ((paramInt1 & 0x400) != 0)); }
...
/*      */       
/*  770 */       return new SeekableFD(this, (Seekable)file, paramInt1, paramFile, file, paramObject) { private final File val$f; private final Seekable.File val$sf; protected Runtime.FStat _fstat() { return this.this$0.hostFStat(this.val$f, this.val$sf, this.val$data); } private final Object val$data; private final Runtime this$0; }
/*      */         ;
/*      */     } 
...
/*  775 */     return null;
/*      */   }
/*      */   FD _open(String paramString, int paramInt1, int paramInt2) throws ErrnoException {
/*  778 */     return hostFSOpen(new File(paramString), paramInt1, paramInt2, null);
/*      */   }
/*      */ 
/*      */   
...
/*      */ 
/*      */   
/*      */   private int sys_write(int paramInt1, int paramInt2, int paramInt3) throws FaultException, ErrnoException {
...
/*      */ 
/*      */   
/*      */   private int sys_read(int paramInt1, int paramInt2, int paramInt3) throws FaultException, ErrnoException {
...
/*  836 */     return closeFD(paramInt) ? 0 : -81;
/*      */   }
/*      */ 
/*      */ 
/*      */   
/*      */   private int sys_lseek(int paramInt1, int paramInt2, int paramInt3) throws ErrnoException {
...
/*  854 */     memWrite(paramInt + 12, paramFStat.gid() << 16 | 0x0);
...
/*  975 */       boolean bool; this.state = 3;
/*      */       
/*      */       try {
/*  978 */         bool = this.callJavaCB.call(paramInt1, paramInt2, paramInt3, paramInt4);
...
/*  997 */     return (this.writePages.length == 1) ? 4096 : (1 << this.pageShift);
/*      */   }
/*      */   
/*      */   void _exited() {}
/*      */   
...
/*      */ 
/*      */   
/*      */   final int sys_fcntl(int paramInt1, int paramInt2, int paramInt3) throws FaultException {
/*      */     int i;
...
/*      */   }
/*      */   
/*      */   int _syscall(int paramInt1, int paramInt2, int paramInt3, int paramInt4, int paramInt5, int paramInt6, int paramInt7) throws ErrnoException, FaultException {
...
/* 1139 */   private int sys_getgid() { return 0; } private int sys_getegid() {
/* 1140 */     return 0;
/*      */   }
/* 1142 */   public int xmalloc(int paramInt) { int i = malloc(paramInt); if (i == 0) throw new RuntimeException("malloc() failed");  return i; }
/* 1143 */   public int xrealloc(int paramInt1, int paramInt2) { int i = realloc(paramInt1, paramInt2); if (i == 0) throw new RuntimeException("realloc() failed");  return i; } public int realloc(int paramInt1, int paramInt2) { 
/* 1144 */     try { return call("realloc", paramInt1, paramInt2); } catch (CallException callException) { return 0; }
/* 1145 */      } public int malloc(int paramInt) { try { return call("malloc", paramInt); } catch (CallException callException) { return 0; }
...
/* 1152 */     byte[] arrayOfByte1 = getBytes(paramString);
/* 1153 */     byte[] arrayOfByte2 = new byte[arrayOfByte1.length + 1];
/* 1154 */     System.arraycopy(arrayOfByte1, 0, arrayOfByte2, 0, arrayOfByte1.length);
/* 1155 */     int i = malloc(arrayOfByte2.length);
/* 1156 */     if (i == 0) return 0; 
/*      */     try {
/* 1158 */       copyout(arrayOfByte2, i, arrayOfByte2.length);
...
/*      */     
/* 1172 */     int i = paramInt;
/* 1173 */     for (int j = 1; j; i++) {
/* 1174 */       j = memRead(i & 0xFFFFFFFC);
/* 1175 */       switch (i & 0x3) { case 0:
/* 1176 */           j = j >>> 24 & 0xFF; break;
/* 1177 */         case 1: j = j >>> 16 & 0xFF; break;
/* 1178 */         case 2: j = j >>> 8 & 0xFF; break;
/* 1179 */         case 3: j = j >>> 0 & 0xFF; break; }
/*      */     
/*      */     } 
...
/* 1203 */         case 2: if ((i >>> 8 & 0xFF) == 0) return stringBuffer.toString();  stringBuffer.append((char)(i >>> 8 & 0xFF)); paramInt++;
/* 1204 */         case 3: if ((i >>> 0 & 0xFF) == 0) return stringBuffer.toString();  stringBuffer.append((char)(i >>> 0 & 0xFF)); paramInt++; }
/*      */     
/*      */     } 
/*      */   }
...
/*      */     
/* 1215 */     public void setNormalizedPath(String param1String) { this.normalizedPath = param1String; } public String getNormalizedPath() {
/* 1216 */       return this.normalizedPath;
/*      */     }
/* 1218 */     public void markDeleteOnClose() { this.deleteOnClose = true; } public boolean isMarkedForDeleteOnClose() {
/* 1219 */       return this.deleteOnClose;
/*      */     }
/*      */     public int read(byte[] param1ArrayOfbyte, int param1Int1, int param1Int2) throws Runtime.ErrnoException {
/* 1222 */       throw new Runtime.ErrnoException(81);
/*      */     } public int write(byte[] param1ArrayOfbyte, int param1Int1, int param1Int2) throws Runtime.ErrnoException {
/* 1224 */       throw new Runtime.ErrnoException(81);
/*      */     }
/*      */     public int seek(int param1Int1, int param1Int2) throws Runtime.ErrnoException {
/* 1227 */       return -1;
/*      */     } public int getdents(byte[] param1ArrayOfbyte, int param1Int1, int param1Int2) throws Runtime.ErrnoException {
/* 1229 */       throw new Runtime.ErrnoException(81);
/*      */     }
/*      */     
/*      */     Seekable seekable() {
/* 1233 */       return null;
...
/* 1245 */       if (--this.refCount == 0) _close(); 
/*      */     } protected void _close() {}
/*      */     FD dup() {
/* 1248 */       this.refCount++; return this;
/*      */     } }
/*      */ 
/*      */   
/*      */   public static abstract class SeekableFD extends FD { private final int flags;
/*      */     private final Seekable data;
/*      */     
/*      */     SeekableFD(Seekable param1Seekable, int param1Int) {
/* 1256 */       this.data = param1Seekable; this.flags = param1Int;
/*      */     } protected abstract Runtime.FStat _fstat();
/*      */     public int flags() {
/* 1259 */       return this.flags;
/*      */     } Seekable seekable() {
/* 1261 */       return this.data;
/*      */     }
/*      */     public int seek(int param1Int1, int param1Int2) throws Runtime.ErrnoException {
/*      */       
/* 1265 */       try { switch (param1Int2)
/*      */         
/*      */         { 
/*      */ 
/*      */           
...
/* 1272 */             return param1Int1;case 1: param1Int1 += this.data.pos();
/* 1273 */           case 2: param1Int1 += this.data.length(); }  return -1; } catch (IOException iOException)
...
/*      */     
/* 1306 */     public InputOutputStreamFD(InputStream param1InputStream) { this(param1InputStream, null); } public InputOutputStreamFD(OutputStream param1OutputStream) {
/* 1307 */       this(null, param1OutputStream);
...
/* 1346 */       return new Runtime.SocketFStat();
/*      */     }
/*      */   }
/*      */   
/* 1350 */   static class TerminalFD extends InputOutputStreamFD { public TerminalFD(InputStream param1InputStream) { this(param1InputStream, null); }
/* 1351 */     public TerminalFD(OutputStream param1OutputStream) { this(null, param1OutputStream); } public TerminalFD(InputStream param1InputStream, OutputStream param1OutputStream) {
/* 1352 */       super(param1InputStream, param1OutputStream);
/*      */     } public void _close() {} public Runtime.FStat _fstat() {
...
/*      */     } }
/*      */   
/* 1359 */   static class Win32ConsoleIS extends InputStream { private int pushedBack = -1; private final InputStream parent;
...
/* 1378 */         if (param1ArrayOfbyte[param1Int1 + b] == 13) {
/* 1379 */           if (b == i - 1) {
/* 1380 */             int j = this.parent.read();
/* 1381 */             if (j == 10) { param1ArrayOfbyte[param1Int1 + b] = 10; }
/* 1382 */             else { this.pushedBack = j; } 
/* 1383 */           } else if (param1ArrayOfbyte[param1Int1 + b + 1] == 10) {
/* 1384 */             System.arraycopy(param1ArrayOfbyte, param1Int1 + b + 1, param1ArrayOfbyte, param1Int1 + b, param1Int2 - b - 1);
/* 1385 */             i--;
/*      */           } 
/*      */         }
/*      */       } 
...
/* 1408 */     public int blksize() { return 512; } public int blocks() {
/* 1409 */       return (size() + blksize() - 1) / blksize();
/*      */     }
...
/* 1418 */     public int type() { return 49152; } public int inode() {
/* 1419 */       return hashCode() & 0x7FFF;
/*      */     } }
/*      */   
/*      */   static class HostFStat extends FStat { private final File f;
/*      */     private final Seekable.File sf;
/*      */     private final boolean executable;
/*      */     
/* 1426 */     public HostFStat(File param1File, Seekable.File param1File1) { this(param1File, param1File1, false); } public HostFStat(File param1File, boolean param1Boolean) {
/* 1427 */       this(param1File, null, param1Boolean);
...
/* 1435 */     public int type() { return this.f.isDirectory() ? 16384 : 32768; } public int nlink() {
/* 1436 */       return 1;
/*      */     } public int mode() {
/* 1438 */       int i = 0;
/* 1439 */       boolean bool = this.f.canRead();
/* 1440 */       if (bool && (this.executable || this.f.isDirectory())) i |= 0x49; 
/* 1441 */       if (bool) i |= 0x124; 
/* 1442 */       if (this.f.canWrite()) i |= 0x92; 
/* 1443 */       return i;
/*      */     }
/*      */     public int size() {
/*      */       try {
...
/*      */       } 
/*      */     } public int mtime() {
/* 1452 */       return (int)(this.f.lastModified() / 1000L);
/*      */     } }
/*      */ 
/*      */   
/*      */   public static class ReadFaultException extends FaultException { public ReadFaultException(int param1Int) {
/* 1457 */       super(param1Int);
/*      */     } }
/*      */   public static class WriteFaultException extends FaultException { public WriteFaultException(int param1Int) {
/* 1460 */       super(param1Int);
/*      */     } }
/*      */   public static class FaultException extends ExecutionException { public final int addr;
/*      */     public final RuntimeException cause;
/*      */     
/* 1465 */     public FaultException(int param1Int) { super("fault at: " + Runtime.toHex(param1Int)); this.addr = param1Int; this.cause = null; } public FaultException(RuntimeException param1RuntimeException) {
/* 1466 */       super(param1RuntimeException.toString()); this.addr = -1; this.cause = param1RuntimeException;
/*      */     } }
/*      */   
/* 1469 */   public static class ExecutionException extends Exception { private String message = "(null)";
...
/* 1473 */     void setLocation(String param1String) { this.location = (param1String == null) ? "(unknown)" : param1String; } public final String getMessage() {
/* 1474 */       return this.message + " at " + this.location;
/*      */     } }
/*      */   public static class CallException extends Exception { public CallException(String param1String) {
/* 1477 */       super(param1String);
/*      */     } }
/*      */   protected static class ErrnoException extends Exception { public int errno;
/*      */     
/*      */     public ErrnoException(int param1Int) {
/* 1482 */       super("Errno: " + param1Int); this.errno = param1Int;
/*      */     } }
/*      */ 
/*      */ 
/*      */   
...
/* 1513 */     public boolean allowStat(File param1File) { return true; } public boolean allowUnlink(File param1File) {
/* 1514 */       return true;
/*      */     }
/*      */   }
/*      */   
/*      */   protected final void nullPointerCheck(int paramInt) throws ExecutionException {
...
/* 1536 */     int i = 0, j = 0;
/* 1537 */     for (byte b1 = 0, b2 = 0; b2 < paramInt; b1 += 8) {
/* 1538 */       long l = 0L;
/* 1539 */       for (byte b = 0; b < 8; ) { l <<= 7L; l |= (paramString.charAt(b1 + b) & 0x7F); b++; }
/* 1540 */        if (j) arrayOfInt[b2++] = i | (int)(l >>> 56 - j); 
/* 1541 */       if (b2 < paramInt) arrayOfInt[b2++] = (int)(l >>> 24 - j); 
/* 1542 */       j = j + 8 & 0x1F;
/* 1543 */       i = (int)(l << j);
/*      */     } 
...
/*      */   }
/*      */   
/* 1563 */   static final String toHex(int paramInt) { return "0x" + Long.toString(paramInt & 0xFFFFFFFFL, 16); }
/* 1564 */   static final int min(int paramInt1, int paramInt2) { return (paramInt1 < paramInt2) ? paramInt1 : paramInt2; } static final int max(int paramInt1, int paramInt2) {
/* 1565 */     return (paramInt1 > paramInt2) ? paramInt1 : paramInt2;
/*      */   }
/*      */   
