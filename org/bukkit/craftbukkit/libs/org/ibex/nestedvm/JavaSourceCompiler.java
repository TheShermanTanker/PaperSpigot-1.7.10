...
/*  23 */     this.out.println();
/*     */   }
/*  25 */   private void p(String paramString) { this.out.println(indents[this.indent] + paramString); } private void pblock(StringBuffer paramStringBuffer) {
/*  26 */     this.out.print(paramStringBuffer.toString());
/*     */   }
/*     */ 
/*     */ 
/*     */   
...
/* 556 */         i9 = paramInt1 & 0xF0000000 | i5 << 2;
...
/*     */         return;
/*     */     } 
/* 894 */     throw new Compiler.Exn("Invalid Instruction: " + i + " at " + toHex(paramInt1)); } private void branch(int paramInt1, int paramInt2) { if (this.debugCompiler) p("lastPC = " + toHex(paramInt1) + ";");  p("pc=" + constant(paramInt2) + ";"); if (paramInt2 == 0) { p("throw new ExecutionException(\"Branch to addr 0x0\");"); } else if ((paramInt1 & this.methodMask) == (paramInt2 & this.methodMask)) { p("continue;"); } else if (this.assumeTailCalls) { p("run_" + Long.toString((paramInt2 & this.methodMask) & 0xFFFFFFFFL, 16) + "(); return;"); } else { leaveMethod(); }  }
/*     */   private void leaveMethod() { p("return;"); }
/*     */   private void emitText(int paramInt1, DataInputStream paramDataInputStream, int paramInt2) throws Compiler.Exn, IOException { if (this.textDone) throw new Compiler.Exn("Multiple text segments");  this.textDone = true; if ((paramInt1 & 0x3) != 0 || (paramInt2 & 0x3) != 0) throw new Compiler.Exn("Section on weird boundaries");  int i = paramInt2 / 4; int j = paramDataInputStream.readInt(); if (j == -1) throw new Error("Actually read -1 at " + toHex(paramInt1));  for (byte b = 0; b < i; b++, paramInt1 += 4) { int k = j; j = (b == i - 1) ? -1 : paramDataInputStream.readInt(); if (paramInt1 >= this.endOfMethod) { endMethod(); startMethod(paramInt1); }  if (this.jumpableAddresses == null || paramInt1 == this.startOfMethod || this.jumpableAddresses.get(new Integer(paramInt1)) != null) { p("case " + toHex(paramInt1) + ":"); this.unreachable = false; } else { if (this.unreachable) continue;  if (this.debugCompiler) p("/* pc = " + toHex(paramInt1) + "*" + "/");  }  this.indent++; emitInstruction(paramInt1, k, j); this.indent--; continue; }  endMethod(paramInt1); p(); paramDataInputStream.close(); }
...
/* 918 */     return "(Float.intBitsToFloat(f" + paramInt + "))";
/*     */   } private static String getDouble(int paramInt) {
/* 920 */     return "(Double.longBitsToDouble(((f" + (paramInt + 1) + "&0xffffffffL) << 32) | (f" + paramInt + "&0xffffffffL)))";
/*     */   } private static String setFloat(int paramInt, String paramString) {
/* 922 */     return "f" + paramInt + "=Float.floatToRawIntBits(" + paramString + ");";
/*     */   } private static String setDouble(int paramInt, String paramString) {
/* 924 */     return "{ long l = Double.doubleToLongBits(" + paramString + "); " + "f" + (paramInt + 1) + " = (int)(l >>> 32); f" + paramInt + " = (int)l; }";
/*     */   }
/*     */ }
