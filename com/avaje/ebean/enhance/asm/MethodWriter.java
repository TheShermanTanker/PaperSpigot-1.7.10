...
/*  469 */     return new AnnotationWriter(this.cw, false, this.annd, null, 0);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  506 */       return new AnnotationWriter(this.cw, false, bv, null, 0);
/*      */     } 
/*      */     
...
/*      */     
/*  551 */     if (type == -1) {
/*  552 */       startFrame(this.code.length, nLocal, nStack); int i;
...
/*      */         }
/*  916 */         this.code.putByte((opcode <= 166) ? ((opcode + 1 ^ 0x1) - 1) : (opcode ^ 0x1));
/*      */ 
/*      */         
...
/* 1210 */         Label label1 = handler.start.getFirst();
...
/* 1225 */           b.successor = h;
/*      */           
/* 1227 */           b.next = label1.successors;
/* 1228 */           label1.successors = b;
/*      */           
/* 1230 */           label1 = label1.successor;
/*      */         } 
...
/*      */         
/* 1251 */         Label label = changed;
/* 1252 */         changed = changed.next;
/* 1253 */         label.next = null;
/* 1254 */         f = label.frame;
/*      */         
/* 1256 */         if ((label.status & 0x10) != 0) {
/* 1257 */           label.status |= 0x20;
/*      */         }
/*      */         
/* 1260 */         label.status |= 0x40;
/*      */         
/* 1262 */         int blockMax = f.inputStack.length + label.outputStackMax;
/* 1263 */         if (blockMax > max) {
/* 1264 */           max = blockMax;
/*      */         }
/*      */         
/* 1267 */         Edge e = label.successors;
/* 1268 */         while (e != null) {
...
/*      */               
/* 1358 */               id++;
/* 1359 */               subroutine.visitSubroutine(null, id / 32L << 32L | 1L << id % 32, this.subroutines);
/*      */             } 
/*      */           } 
/*      */           
/* 1363 */           l = l.successor;
...
/* 2112 */         int label, newOffset, opcode = b[u] & 0xFF;
/* 2113 */         int insert = 0;
/*      */         
...
/*      */     
/* 2254 */     int j = 0;
/* 2255 */     while (j < this.code.length) {
/* 2256 */       int label, newOffset, v, k, opcode = b[j] & 0xFF;
...
/* 2269 */             label = j + readUnsignedShort(b, j + 1);
/*      */           } else {
/* 2271 */             label = j + readShort(b, j + 1);
/*      */           } 
/* 2273 */           newOffset = getNewOffset(allIndexes, allSizes, j, label);
/* 2274 */           if (resize[j]) {
/*      */ 
/*      */ 
/*      */ 
...
/*      */             } else {
/* 2285 */               newCode.putByte((opcode <= 166) ? ((opcode + 1 ^ 0x1) - 1) : (opcode ^ 0x1));
/*      */ 
/*      */               
...
/*      */           continue;
/*      */         case 9:
/* 2301 */           label = j + readInt(b, j + 1);
/* 2302 */           newOffset = getNewOffset(allIndexes, allSizes, j, label);
...
/* 2314 */           label = v + readInt(b, j);
/* 2315 */           j += 4;
/* 2316 */           newOffset = getNewOffset(allIndexes, allSizes, v, label);
/* 2317 */           newCode.putInt(newOffset);
/* 2318 */           k = readInt(b, j);
/* 2319 */           j += 4;
/* 2320 */           newCode.putInt(k);
/* 2321 */           k = readInt(b, j) - k + 1;
/* 2322 */           j += 4;
/* 2323 */           newCode.putInt(readInt(b, j - 4));
/* 2324 */           for (; k > 0; k--) {
/* 2325 */             label = v + readInt(b, j);
/* 2326 */             j += 4;
...
/* 2338 */           label = v + readInt(b, j);
/* 2339 */           j += 4;
/* 2340 */           newOffset = getNewOffset(allIndexes, allSizes, v, label);
/* 2341 */           newCode.putInt(newOffset);
/* 2342 */           k = readInt(b, j);
/* 2343 */           j += 4;
/* 2344 */           newCode.putInt(k);
/* 2345 */           for (; k > 0; k--) {
/* 2346 */             newCode.putInt(readInt(b, j));
/* 2347 */             j += 4;
/* 2348 */             label = v + readInt(b, j);
/* 2349 */             j += 4;
...
/*      */           continue;
/*      */         case 16:
/* 2355 */           opcode = b[j + 1] & 0xFF;
/* 2356 */           if (opcode == 132) {
/* 2357 */             newCode.putByteArray(b, j, 6);
/* 2358 */             j += 6; continue;
/*      */           } 
/* 2360 */           newCode.putByteArray(b, j, 4);
/* 2361 */           j += 4;
/*      */           continue;
/*      */         
/*      */         case 1:
/*      */         case 3:
/*      */         case 10:
/* 2367 */           newCode.putByteArray(b, j, 2);
/* 2368 */           j += 2;
/*      */           continue;
/*      */         case 2:
/*      */         case 5:
...
/*      */         case 11:
/*      */         case 12:
/* 2375 */           newCode.putByteArray(b, j, 3);
/* 2376 */           j += 3;
/*      */           continue;
/*      */         case 7:
/* 2379 */           newCode.putByteArray(b, j, 5);
/* 2380 */           j += 5;
/*      */           continue;
/*      */       } 
/*      */       
/* 2384 */       newCode.putByteArray(b, j, 4);
/* 2385 */       j += 4;
/*      */     } 
/*      */ 
/*      */ 
...
/*      */           
/* 2408 */           j = l.position - 3;
/* 2409 */           if ((l.status & 0x20) != 0 || (j >= 0 && resize[j])) {
/*      */             
...
/* 2448 */         b = bv.data;
/* 2449 */         j = 0;
/* 2450 */         while (j < bv.length) {
/* 2451 */           int k = readUnsignedShort(b, j);
/* 2452 */           int m = getNewOffset(allIndexes, allSizes, 0, k);
/* 2453 */           writeShort(b, j, m);
/* 2454 */           k += readUnsignedShort(b, j + 2);
/* 2455 */           m = getNewOffset(allIndexes, allSizes, 0, k) - m;
/*      */           
/* 2457 */           writeShort(b, j + 2, m);
/* 2458 */           j += 10;
/*      */         } 
/*      */       } 
/*      */     } 
...
/* 2464 */       j = 0;
/* 2465 */       while (j < this.lineNumber.length) {
/* 2466 */         writeShort(b, j, getNewOffset(allIndexes, allSizes, 0, readUnsignedShort(b, j)));
/*      */ 
/*      */ 
/*      */         
/* 2470 */         j += 4;
...
/* 2519 */     return (b[index] & 0xFF) << 24 | (b[index + 1] & 0xFF) << 16 | (b[index + 2] & 0xFF) << 8 | b[index + 3] & 0xFF;
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/* Location:              D:\Paper-1.7.10\PaperSpigot-1.7.10-R0.1-SNAPSHOT-latest.jar!\com\avaje\ebean\enhance\asm\MethodWriter.class
