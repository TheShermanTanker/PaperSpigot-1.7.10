...
/*     */ 
/*     */   
/*  40 */   private final List<DRawSqlColumnInfo> columns = new ArrayList<DRawSqlColumnInfo>();
/*     */   
...
/* 106 */     if (prop == null) {
/* 107 */       if (this.debug) {
/* 108 */         String str = "ColumnMapping ... idx[" + this.columnIndex + "] ERROR, no property found to match... column[" + colName + "] label[" + colLabel + "] search[" + this.searchColumn + "]";
/*     */ 
/*     */         
/* 111 */         this.parent.debug(str);
/*     */       } 
...
/* 189 */     for (int i = 0; i < propertiesBase.length; i++) {
...
/* 197 */     for (int j = 0; j < propertiesId.length; j++) {
/* 198 */       BeanProperty prop = propertiesId[j];
/* 199 */       if (isMatch(prop, this.searchColumn)) {
/* 200 */         return prop;
/*     */       }
/*     */     } 
/*     */     
/* 204 */     BeanPropertyAssocOne[] arrayOfBeanPropertyAssocOne = (BeanPropertyAssocOne[])this.desc.propertiesOne();
/* 205 */     for (int k = 0; k < arrayOfBeanPropertyAssocOne.length; k++) {
/* 206 */       BeanProperty prop = arrayOfBeanPropertyAssocOne[k];
...
/* Location:              D:\Paper-1.7.10\PaperSpigot-1.7.10-R0.1-SNAPSHOT-latest.jar!\com\avaje\ebeaninternal\server\deploy\DRawSqlSelectColumnsParser.class
