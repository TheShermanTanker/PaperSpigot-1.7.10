...
/*     */   public PstmtCache getPstmtCache() { return this.pstmtCache; }
/*     */   public Statement createStatement() throws SQLException { if (this.status == 88)
/*     */       throw new SQLException(IDLE_CONNECTION_ACCESSED_ERROR + "createStatement()");  try { return this.connection.createStatement(); } catch (SQLException ex) { addError(ex); throw ex; }  }
...
/* 617 */      } public void setReadOnly(boolean readOnly) throws SQLException { this.resetIsolationReadOnlyRequired = true;
/* 618 */     this.connection.setReadOnly(readOnly); } protected void returnPreparedStatement(ExtendedPreparedStatement pstmt) { synchronized (this.pstmtMonitor) { ExtendedPreparedStatement alreadyInCache = this.pstmtCache.get(pstmt.getCacheKey()); if (alreadyInCache == null) { this.pstmtCache.put(pstmt.getCacheKey(), pstmt); } else { try { pstmt.closeDestroy(); } catch (SQLException e) { logger.log(Level.SEVERE, "Error closing Pstmt", e); }  }  }  }
/*     */   public PreparedStatement prepareStatement(String sql, int returnKeysFlag) throws SQLException { String cacheKey = sql + returnKeysFlag; return prepareStatement(sql, true, returnKeysFlag, cacheKey); }
/*     */   public PreparedStatement prepareStatement(String sql) throws SQLException { return prepareStatement(sql, false, 0, sql); }
...
/*     */       throw new SQLException(IDLE_CONNECTION_ACCESSED_ERROR + "prepareStatement()");  try { this.pstmtMissCounter++; this.lastStatement = sql; return this.connection.prepareStatement(sql, resultSetType, resultSetConcurreny); } catch (SQLException ex) { addError(ex); throw ex; }  }
...
/* 626 */   public void setTransactionIsolation(int level) throws SQLException { if (this.status == 88) {
...
/* 633 */     { addError(ex);
/* 634 */       throw ex; }  } public boolean hadErrors() { return this.hadErrors; }
/*     */   public void close() throws SQLException { if (this.status == 88) throw new SQLException(IDLE_CONNECTION_ACCESSED_ERROR + "close()");  if (this.hadErrors && !this.pool.validateConnection(this)) { closeConnectionFully(false); this.pool.checkDataSource(); return; }  try { if (this.connection.getAutoCommit() != this.pool.getAutoCommit())
/*     */         this.connection.setAutoCommit(this.pool.getAutoCommit());  if (this.resetIsolationReadOnlyRequired) { resetIsolationReadOnly(); this.resetIsolationReadOnlyRequired = false; }  this.lastUseTime = System.currentTimeMillis(); this.status = 88; this.pool.returnConnection(this); } catch (Exception ex) { closeConnectionFully(false); this.pool.checkDataSource(); }  }
/*     */   private void resetIsolationReadOnly() throws SQLException { if (this.connection.getTransactionIsolation() != this.pool.getTransactionIsolation())
...
/*     */   protected void finalize() throws Throwable { try { if (this.connection != null && !this.connection.isClosed()) { String msg = "Closing Connection[" + getName() + "] on finalize()."; logger.warning(msg); closeConnectionFully(false); }  } catch (Exception e) { logger.log(Level.SEVERE, (String)null, e); }  super.finalize(); }
/*     */   public long getStartUseTime() { return this.startUseTime; }
...
/* 646 */   public void clearWarnings() throws SQLException { if (this.status == 88) {
...
/*     */       }
/*     */     } 
/* 957 */     return filteredList.<StackTraceElement>toArray(new StackTraceElement[filteredList.size()]);
/*     */   }
/*     */ }


/* Location:              D:\Paper-1.7.10\PaperSpigot-1.7.10-R0.1-SNAPSHOT-latest.jar!\com\avaje\ebeaninternal\server\lib\sql\PooledConnection.class
